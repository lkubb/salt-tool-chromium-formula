...
We will see a quick setup to configure the ``tool_chromium`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_chromium`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_chromium`` formulas under ``/srv/salt/tool_chromium/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_chromium/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_chromium/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_chromium/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_chromium/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_chromium`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_chromium`` formulas under ``/srv/salt/tool_chromium/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_chromium/parameters/roles

We will define 2 roles:

- ``tool_chromium/server``
- ``tool_chromium/client``

We create a configuration for the role ``tool_chromium/server`` in ``/srv/salt/tool_chromium/parameters/roles/tool_chromium/server.yaml``:
...
We create another configuration for the role ``tool_chromium/client`` in ``/srv/salt/tool_chromium/parameters/roles/tool_chromium/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_chromium/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_chromium/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_chromium/parameters/dns:domain/  '/srv/salt/tool_chromium/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-chromium-as-json:
      test.nop:
        - name: {{ chromium | json }}
...
.. code-block:: sls
    ...
    chromium-config-file-file-managed:
      file.managed:
        - name: {{ chromium.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='chromium-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``chromium`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ chromium['winner'] }}
